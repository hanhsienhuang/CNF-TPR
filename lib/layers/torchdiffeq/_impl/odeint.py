...
from .heun_euler import HeunEuler
...
class OdeintStep:
    def __init__(self, rtol=1e-7, atol=1e-9, method=None, options=None):
        ...
        if method is None:
            method = 'dopri5'
        self.method, self.rtol, self.atol, self.options = method, rtol, atol, options
        self.solver = None

    def step(self, func, y0, t):
        tensor_input, func, y0, t = _check_inputs(func, y0, t)

        if self.solver is None:
            method, rtol, atol, options = self.method, self.rtol, self.atol, self.options 
            self.solver = SOLVERS[method](func, y0, rtol=rtol, atol=atol, **options)
        self.solver.y0 = y0
        self.func = func
        solution = self.solver.integrate(t)
